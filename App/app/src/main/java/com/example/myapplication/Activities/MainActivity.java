...
import com.example.myapplication.Adapters.SlidesAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void  initUpcomingMovie(){
    ...
    private void initBanner(){
        ...
        ArrayList<SlideItems> items = new ArrayList<>();
        myRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    for(DataSnapshot issue:snapshot.getChildren()){
                        items.add(issue.getValue(SlideItems.class));
                    }
                    banners(items);
    ...
    private void banners(ArrayList<SlideItems> items){
        binding.viewPager2.setAdapter(new SlidesAdapter(items,binding.viewPager2));
